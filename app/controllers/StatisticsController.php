...
class StatisticsController extends Controller
{
    ...
    private function getAccountSummary()
    {
        ...
        $username = $ActiveAccount->get("username");
        $resp = [];
        try {
            $igresp = InstagramController::getAccountInfo($ActiveAccount);
            $resp["media_count"] = empty($igresp->user->media_count) ? 0 : $igresp->user->media_count;
            $resp["following_count"] = empty($igresp->user->following_count) ? 0 : $igresp->user->following_count;
            $resp["follower_count"] = empty($igresp->user->follower_count) ? 0 : $igresp->user->follower_count;
        } catch (Exception $e) {
