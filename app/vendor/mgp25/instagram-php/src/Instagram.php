...
class Instagram
{
        ...
        if (!$this->isLoggedIn || $forceLogin) {
            ...
            try {
                $response = $this->request('fb/facebook_signup/')
                    ...
                    ->addPost('guid', $this->uuid)
                    ->addPost('device_id', $this->device_id)
                    ->addPost('fb_access_token', $this->password)
                    ->addPost('login_attempt_count', 0)
                    ->getResponse(new Response\LoginResponse());
            } catch (\InstagramAPI\Exception\InstagramException $e) {
                if ($e->hasResponse() && $e->getResponse()->getTwoFactorRequired()) {
    ...
    public function sendChallengeVerificationCode(
        $instagram_id,
        $challenge_id,
        $choice = 1,
        $replay = false)
    {
        ...
        if ($replay) {
            $response = $this->request('https://i.instagram.com/api/v1/challenge/replay'.$instagram_id.'/'.$challenge_id.'/') 
                ->setNeedsAuth(false)
                ->getResponse(new Response\LoginResponse());
        } else {
            $response = $this->request('https://i.instagram.com/api/v1/challenge/'.$instagram_id.'/'.$challenge_id.'/') 
                ->setNeedsAuth(false)
    ...
    public function approveChallengeVerificationCode(
        $instagram_id,
        $challenge_id,
        $security_code)
    {
