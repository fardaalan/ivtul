...
var NextPost = {};
    ...
    {   
        ...
        $("body").on("submit", ".js-ajax-form2", function(){
            $form = $(this);
            $result = $form.find(".form-result")
            submitable = true;
    ...
    NextPost.Account = function()
    {
        $(document).ajaxComplete(function(event, xhr, settings) {
            ...
            if (!xhr.responseJSON.hasOwnProperty("links")) {
                return;
            }

            if (xhr.responseJSON.links.length == 2) {
                $.confirm({
                    ...
                    buttons: {
                        sms: {
                            text: xhr.responseJSON.links[0].label,
                            btnClass: "small button mr-5",
                            action: function() {
                                $("body").addClass("onprogress");
                                window.location.href = xhr.responseJSON.links[0].uri
                            }
                        },

                        email: {
                            text: xhr.responseJSON.links[1].label,
                            btnClass: "small button",
                            action: function() {
                                $("body").addClass("onprogress");
                                window.location.href = xhr.responseJSON.links[1].uri
                            }
                        }
                    }
                });
