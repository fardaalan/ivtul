...
function _comment_target_feed($sc, $Instagram, $comments, $do_spintax = false)
{
    ...
    if ($target->type == "hashtag") {
        ...
        foreach ($feed->items as $item) {
            if (!empty($item->id) && empty($item->comments_disabled))  {
                $_log = new LogModel([
                    "user_id" => $sc->get("user_id"),
                    "account_id" => $sc->get("account_id"),
                    "commented_media_code" => $item->code,
                    "status" => "success"
                ]);
...
function _comment_timeline_feed($sc, $Instagram, $comments, $do_spintax = false)
{
    ...
    while ($loop) {
        ...
        if (empty($feed->feed_items[0]->id)) {
            // Invalid
            $loop = false;
        }

        $items = array_merge($items, $feed->feed_items);
        $round++;
    ...
    foreach ($items as $item) {
        if (!empty($item->id) && empty($item->comments_disabled))  {
            ...
            if (!$_log->isAvailable()) {
                ...
                try {
                    $resp = $Instagram->media->comment($item->id, $comment);
                } catch (\Exception $e) {
                    continue;
                }
                ...
                $_log->set("status", "success")
                     ...
                     ->set("commented_media_code", $item->code)
                     ->save();
...
function _get_media_thumb_igitem($item)
{
    ...
    $media_type = empty($item->media_type) ? null : $item->media_type;
    ...
    if ($media_type == 1 || $media_type == 2) {
        ...
        // ALbum
        if (isset($item->carousel_media[0]->image_versions2->candidates[0]->url)) {
            $media_thumb = $item->carousel_media[0]->image_versions2->candidates[0]->url;
        }
    }
